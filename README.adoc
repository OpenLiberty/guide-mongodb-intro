...
The [hotspot=stageTwo file=0]`second stage` of the Dockerfile uses the [hotspot=mongo file=0]`mongo` image as the base image and creates the [hotspot=createDirectory]`/etc/mongodb` directory. Then, it copies over the generated files from the [hotspot=stageOne hotspot=copyCerts file=0]`first stage`. Additionally, it copies the [hotspot=copyConfig file=0]`mongodb.conf` configuration file and the [hotspot=copyJS file=0]`index.js` script from your host machine to the image. Then, it starts the `MongoDB` daemon using the [hotspot=runMongod file=0]`mongod` command and specifying the [hotspot=fork file=0]`fork` flag to run the daemon as a background process and the [hotspot=config file=0]`config` flag to specify the config the daemon should use. While the `MongoDB` daemon is running in the background, the [hotspot=copyJS file=0]`index.js` script is run using the [hotspot=runScript file=0]`mongo` command which connects to the `MongoDB` daemon. The [hotspot=testdb file=0]`testdb` argument specifies to use the `testdb` database, the [hotspot=tls file=0]`tls` flag specifies to connect to the `MongoDB` daemon using TLS, the [hotspot=tlsCAFile file=0]`tlsCAFile` flag specifies the file to validate the certificate presented by the `MongoDB` daemon, the [hotspot=host file=0]`host` flag is used to verified the Common Name (CN) in the certificate presented by the `MongoDB` daemon, and the path to the [hotspot=script file=0]`index.js` script is given in the last argument. The [hotspot file=1]`index.js` script creates a new [hotspot=createUser file=1]`user` in the `testdb` database with a username of `sampleUser`, password of `openliberty`, and the role of https://docs.mongodb.com/manual/reference/built-in-roles/#readWrite[readWrite]. It also creates a [hotspot=createCollection file=1]`Crew` collection in the `testdb` database to be used by the application. The `MongoDB` daemon is then [hotspot=shutdown file=0]`shutdown` and [hotspot=chown file=0]`chown` is called to change the ownership of the `/etc/mongodb` directory to `mongodb`. Lastly, the [hotspot=cmd file=0]`CMD` command is run to specify the entry point of the Docker image which will start the `MongoDB` daemon using the `mongodb.conf` configuration file.
