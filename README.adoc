...
Properties such as the [hotspot=mongo.hostname file=1]`hostname`, `port`, and `database name` for the running `MongoDB` instance are set in this file. Additionally, the user’s `username` and `password` are set here. For added security, the password was encoded by using the `securityUtility` encode command.
