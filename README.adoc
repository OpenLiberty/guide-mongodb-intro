...
Additional details and configuration options can be found in the official https://docs.mongodb.com/manual/reference/configuration-options/[MongoDB website^]. For more information about the mongo docker image, see https://hub.docker.com/_/mongo[mongo^] in Docker Hub.
...
Properties such as the [hotspot=hostname file=2]`hostname`, [hotspot=port file=2]`port`, and [hotspot=dbname file=2]`database name` for the running `MongoDB` instance are set. Additionally, the user's [hotspot=mongoUser file=2]`username` and [hotspot=passEncoded file=2]`password` are set here. For added security, the password was encoded using `securityUtility`.
...
A [hotspot=beanValidator]`Validator` is used to validate the [hotspot=crewMember file=1]`CrewMember` using the [hotspot=getViolations]`getViolations()` method before inserting or updating the database by using the [hotspot=add]`add()` or [hotspot=update]`update()` method respectively.
...
The [hotspot=retrieve]`retrieve()` method handles the implementation of `READ`. Firstly the `Crew` collection is retrieved using [hotspot=getCollectionRead]`MongoDatabase.getCollection()`. The [hotspot=find]`MongoCollection.find()` method is invoked to retrieve a `FindIterable<TDocument>` generic using the `Document` type, which is iterable for all the crew members documents in the collection. Each document is concatenated into a String array to be returned.
...
You learned how to access MongoDB by using Contexts and Dependency Injection (CDI), and configure MongoDB through MicroProfile Config.
