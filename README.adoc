...
Execute the following command to copy `truststore.p12` from the container the your host machine:

[role='command']
```
docker cp mongo-guide:/etc/mongodb/truststore.p12 assets
```

Execute the following commands to copy the `truststore.p12` file to the correct location in the `start` and `finish` projects.

[role='command']
```
cp assets/truststore.p12 start/src/main/liberty/config/resources/security/
cp assets/truststore.p12 finish/src/main/liberty/config/resources/security/
```
