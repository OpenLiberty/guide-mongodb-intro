...
:page-description: Learn how to persist data with MongoDB by using Contexts and Dependency Injection (CDI), and configure MongoDB through MicroProfile Config.
...
:page-seo-title: Persisting data with MongoDB
:page-seo-description: A getting started tutorial with examples on how to persist data with MongoDB with Open Liberty by using Contexts and Dependency Injection (CDI), and configure MongoDB through MicroProfile Config.
...
Additional details and configuration options can be found in the official https://docs.mongodb.com/manual/reference/configuration-options/[MongoDB website^]. For more information about the `mongo` docker image, see https://hub.docker.com/_/mongo[mongo^].
...
A [hotspot=beanValidator]`Validator` is used to validate the [hotspot=crewMember file=1]`CrewMember` using the [hotspot=getViolations]`getViolations()` method before `inserting` or `updating` the database.
