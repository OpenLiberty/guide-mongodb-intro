...
**Adding the truststore to the Open Liberty instance**

The truststore that's created in the container needs to be added to the Open Liberty instance so that the server can trust the certificate that MongoDB presents when they connect. Run the following command to copy the `truststore.p12` file from the container to the `start` and `finish` directories:
...
To create a CDI producer for MongoDB and connect over TLS, the Open Liberty instance needs to be correctly configured.
