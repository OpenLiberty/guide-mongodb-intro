...
:page-description: Learn how to create an application with MongoDB and Open Liberty.
...
You will learn how to build and test a simple web application with MongoDB, which will expose
...
The web application also responds to a `GET` request with a JSON representation of the crew members information, where
...
Before you begin, you must install MongoDB onto your machine and setup a new database for the web application.
...
// =================================================================================================
// Creating a web application which stores data to MongoDB
// =================================================================================================
...
The values in the `microprofile-config.properties` file are injected into the [hotspot=mongoProducerInjections]`MongoProducer` class.

=== Using an encoded password with passwordUtilities-1.0

One of the advantages of using a CDI producer is that it can be tailored to your needs. For improved security, the `createMongo` method includes authentication with a [hotspot=username]`user name` and an [hotspot=encodedPassword]`encoded password` which is decoded using [hotspot=passwordUtil]`passwordUtilities-1.0`. This tool is specified in `pom.xml` as a [hotspot=passwordUtilDependency]`dependency` and in `server.xml` as a [hotspot=passwordUtilFeature]`feature`.
...
You will be implementing the basic `CRUD` operations which are `CREATE`, `READ`, `UPDATE`, and `DELETE`. The [http://mongodb.github.io/mongo-java-driver/3.8/javadoc/com/mongodb/client/package-summary.html]`com.mongodb.client` package is used help to implement these `CRUD` operations for the web application. The [http://mongodb.github.io/mongo-java-driver/3.8/javadoc/com/mongodb/client/MongoDatabase.html#getCollection(java.lang.String)]`MongoDatabase.getCollection()` method will be used to get the [http://mongodb.github.io/mongo-java-driver/3.8/javadoc/com/mongodb/client/MongoCollection.html]`MongoCollection` and several methods from [http://mongodb.github.io/mongo-java-driver/3.8/javadoc/com/mongodb/client/MongoCollection.html]`MongoCollection` will be used to interact with the collection to implement the `CRUD` operations.
...
A [hotspot=beanValidator]`Validator` is used to validate the [hotspot=crewMember]`CrewMember` before inserting or updating the database.
...
=== Update
The [http://mongodb.github.io/mongo-java-driver/3.8/javadoc/com/mongodb/client/MongoCollection.html#replaceOne(org.bson.conversions.Bson,TDocument)]`MongoCollection.replaceOne()` method is used update a specified document. This method takes in a `Document` which contains the `id` of the document to be replaced and another `Document` which is the new configuration of the crew member. The method returns an [http://mongodb.github.io/mongo-java-driver/3.8/javadoc/com/mongodb/client/result/UpdateResult.html]`UpdateResult` object with a method called `getMatchedCount()` which indicates the number of matches to the query. The [hotspot=update]`update()` method handles the implementation of `UPDATE`
...
=== Delete
The [http://mongodb.github.io/mongo-java-driver/3.8/javadoc/com/mongodb/client/MongoCollection.html#deleteOne(org.bson.conversions.Bson)]`MongoCollection.deleteOne()` method is used to remove a document from the collection. This method takes in a `Document` which contains the `id` of the document to be deleted and returns a [http://mongodb.github.io/mongo-java-driver/3.8/javadoc/com/mongodb/client/result/DeleteResult.html]`DeleteResult` object with a method called `getDeletedCount()` to indicate the number of documents deleted from the collection. The [hotspot=delete]`remove()` method handles the implmentation of `DELETE`
...
You can test this web application manually by sending HTTP requests to http://localhost:9080/mongo[^] or by starting a server and pointing a web browser at the
...
You developed a Web application in Open Liberty by using MongoDB, JAX-RS and JSON-B.
