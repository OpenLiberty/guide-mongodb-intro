...
:page-description: Learn how to store data with MongoDB using using Contexts and Dependency Injection (CDI) and MicroProfile Config.
...
This guide will use Docker to run an instance of `MongoDB` for a fast setup. A multi-stage Dockerfile which uses `mongo` as the final stage's base and will gather the required configuration files. Then, the resulting `mongo` image will be run in a Docker container and you will need to setup a new database for the microservice. Lastly, the truststore generated in the Docker image will be copied from the container and placed into the Open Liberty server.
...
The [hotspot=stageOne file=0]`first stage` of the Dockerfile uses [hotspot=openLiberty file=0]`open-liberty` as the base to create a [hotspot=generateCerts file=0]`self-signed certificate and private key` which are concatenated together to generate the [hotspot=catCerts file=0]`mongodb_tls.pem` file used for TLS. Then, it uses the Java [hotspot=keytool file=0]`keytool` to generate the truststore and adds the certificate file to the truststore. The [hotspot=stageTwo file=0]`second stage` of the Dockerfile uses [hotspot=mongo file=0]`mongo` as the base and [hotspot=copyCerts file=0]`copies` over the generated files from the [hotspot=stageOne]`previous stage`. Additionally, it copies the [hotspot=copyConfig]`mongodb.conf` configuration file from your host machine to the image.
...
The [hotspot=create]`add()` function handles the implementation of `CREATE`. An instance of `MongoCollection` is retrieved by invoking [hotspot=getCollection]`MongoDatabase.getCollection` method and specifying the `Crew` collection. The `MongoCollection` is a generic with a `TDocument` type parameter and the [hotspot=bsonDocument]`Document` class will be used as the type for this guide. The crew member is converted to a [createCrewMember]`Document` and the [hotspot=insertOne]`MongoCollection#insertOne` method is called to insert the new crew member `Document`.
...
Running it.io.openliberty.guides.application.CrewApplicationEndpointIT
   ...
   === Removing 2 crew members from the database. ===
      === Done. ===
Tests run: 4, Failures: 0, Errors: 0, Skipped: 0, Time elapsed: 0.411 s - in it.io.openliberty.guides.application.CrewApplicationEndpointIT
Results:
