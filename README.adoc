...
This generates an AES encoding of the password `openliberty`. Copy the output and paste it to [hotspot=mongoEncodedPass file=0]`mongo.pass.encoded` property.
...
The [hotspot=createDB file=0]`createDB()` method is annotated with [hotspot=produces2 file=0]`@Produces` and returns an instance of `MongoDatabase`. The method injects the `MongoClient` in its parameters and invokes the [hotspot=getDatabase file=0]`MongoClient.getDatabase()` method, passing in the database name to get the `MongoDatabase` instance. The `MongoDatabase` instance is returned.

The [hotspot=close file=0]`close()` method is a cleanup function for the `MongoClient`.
