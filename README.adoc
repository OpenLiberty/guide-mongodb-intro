...
:page-releasedate: 2020-10-12
:page-essential: true
:page-essential-order: 1
:page-description: Learn how to persist data in Java microservices to MongoDB, a document-oriented database.
...
:page-seo-title: Persisting data in your Java microservices to MongoDB, a document-oriented database
:page-seo-description: A getting started tutorial with examples on how to connect, access and persist data to MongoDB, a document-oriented database, in Java cloud-native applications using Contexts and Dependency Injection (CDI) and Eclipse MicroProfile Config with Open Liberty.
...
Learn how to persist data in your microservices to MongoDB, a document-oriented database.
...
This microservice connects to `MongoDB` by using Transport Layer Security (TLS) and injects a `MongoDatabase` instance into the service through Contexts and Dependency Injection (CDI) producer. Additionally, MicroProfile Config is used to easily configure the `MongoDB` driver.
...
After you replace the `server.xml` file, Open Liberty automatically reloads its configuration. 

The features that are required to create the CDI producer for `MongoDB` are `cdi-2.0`, `ssl-1.0`, `mpConfig-1.4`, and `passwordUtilities-1.0`, which are all specified in the [hotspot=featureManager file=0]`featureManager` element. The Secure Sockets Layer (SSL) context is configured in the [hotspot file=0]`server.xml` file so that the application can connect to `MongoDB` with TLS. The [hotspot=keyStore file=0]`keyStore` element is configured to point to the `truststore.p12` keystore file that was created in one of the earlier sections. The [hotspot=ssl file=0]`ssl` element is configured to use the `defaultKeyStore` as the keystore and `outboundTrustStore` as the truststore.

Using MicroProfile Config makes configuring the `MongoDB` driver simple because all of the configuration can be set in one place and injected into the CDI producer.
...
Properties such as the `hostname`, `port`, and `database name` for the running `MongoDB` instance are set in this file. Additionally, the user’s `username` and `password` are set here. For added security, the password was encoded by using the `securityUtility` encode command.
...
This class contains a `createMongo()` producer method for the `MongoClient`, a `createDB()` producer method for the `MongoDatabase` which depends on the `MongoClient`, and a `close()` dispose method for the `MongoClient`. Before you create the methods, the values in the [hotspot file=1]`microprofile-config.properties` file are injected into the [hotspot=mongoProducerInjections file=0]`MongoProducer` class.

The [hotspot=createMongo file=0]`createMongo()` method returns an instance of `MongoClient`. In this method, the `username`, `database name`, and `decoded password` are passed into the [hotspot=createCredential file=0]`MongoCredential.createCredential()` method to get an instance of `MongoCredential`. The [hotspot=sslContext file=0]`JSSEHelper` is used to get the `SSLContext` from the `outboundSSLContext` in the `server.xml` file. Then, a [hotspot=mongoClient file=0]`MongoClient` instance is created from the above and returned.

The [hotspot=createDB file=0]`createDB()` method returns an instance of `MongoDatabase`. This method injects the `MongoClient` in its parameters and passes the database name into the [hotspot=getDatabase file=0]`MongoClient.getDatabase()` method to get a `MongoDatabase` instance.

The [hotspot=close file=0]`close()` method is a cleanup function for the `MongoClient` that closes the connection to the `MongoDB` instance.
...
You will receive a response that contains the JSON object of the new crew member, as shown in the following example:
[role="no_copy"]
```
{
  "Name": "Member1",
...
You will receive a response that contains an array of all the members in your crew. The response might include crew members that were created in the **Try what you’ll build** section:
[role="no_copy"]
```
[
  {
...
The [hotspot=update]`update()` method handles the implementation of the `update` operation. After `Crew` collection is retrieved, a document is created with the specified object id to query the collection. Next, a new crew member [hotspot=crewMemberUpdate]`Document` is created with the updated configuration. The [hotspot=replaceOne]`MongoCollection.replaceOne()` method is called with the query and new crew member document, which updates all matching queries with the new document. Because the object id is unique in the collection, only one document is updated. `The MongoCollection.replaceOne()` method also returns an UpdateResult, which determines how many documents matched the query. If the match count is zero, then the object id doesn't exist.
...
You will receive a response that contains the JSON object of the updated crew member, as shown in the following example:
...
The [hotspot=remove]`remove()` method handles the implementation of the `delete` operation. After the `Crew` collection is retrieved, a [hotspot=queryDelete]`document` is created with the specified object id to query the collection. Because the object id is unique in the collection, only one document is deleted. After the document is deleted, the [hotspot=deleteOne]`MongoCollection.deleteOne()` method returns a `DeleteResult`, which determines how many documents were deleted. If the deleted count is zero, then the object id doesn't exist.
...
You will receive a response that contains the object id of the deleted crew member, as shown in the following example:
...
You can check out the microservice that you created by going to the http://localhost:9080/mongo/[^] URL after you see a message similar to the following example:
...
Next, you'll create integrated tests to ensure that the basic operations that you implemented function correctly.
...
You learned how to access MongoDB by using Contexts and Dependency Injection (CDI), and configure MongoDB with MicroProfile Config.
