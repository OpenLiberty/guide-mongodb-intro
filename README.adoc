...
- Install https://help.github.com/articles/set-up-git/[Git] and become familiar with the Git triangular workflow.
- Install and know the basics of https://maven.apache.org/index.html[Maven].
- Install a text editor with the https://asciidoctor.org/[AsciiDoc] compiler plugin and learn the syntax of AsciiDoc.

### Step 2 (Optional): Try some existing guides.

Use the guides on https://openliberty.io/guides/[openliberty.io] as examples. 

If you're not sure where to start, try the https://openliberty.io/guides/rest-intro.html[REST Guide] followed by the  https://openliberty.io/guides/cdi-intro.html[MicroProfile tutorial].
...
> A. Develop code and write tests that are based on the project structure in the {template-url}[`guides-template`]. To start, create your project in the `finish` directory. Make a simple application that demonstrates how to use your topic.
...
```
INSTRUCTION: Please remove all the instructions before APPENDIX A prior to commit. Most comments should be removed, although not the copyright. In order to see all the comments, please open this README.adoc in raw format.
...
// Example title: Creating a REST API for a todo list application
...
Learn how to .... using ... and Open Liberty.
...
// EXAMPLE: The following block demostrate how different sections should look like for a to do application.
////
== Building the model
...
////
EXAMPLE: Create the todo test class in the `finish/src/test/java/it/io/openliberty/guides/todo/TodoTest.java` file:
[source, Java]
----
...
* OPTIONAL: after listing the test results, mention a simple change a user can make/introduce that will cause the tests to fail. Be brief and don't give the users all of the instructions. At this point, they should be comfortable enough to figure it out on their own.
