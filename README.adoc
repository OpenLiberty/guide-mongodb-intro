...
First, inject the values in the [hotspot file=1]`microprofile-config.properties` file into the [hotspot=mongoProducerInjections file=0]`MongoProducer` class. Then, create the following methods for the `MongoClient`:
...
The [hotspot=close file=0]`close()` dispose method is a cleanup function for the `MongoClient` that closes the connection to the `MongoDB` instance.
