...
Before you begin, you must generate a self-signed certificate, add it to the truststore, run `MongoDB` in a Docker container, and setup a new database for the microservice.

=== Generating a self-signed certificate

You will be generating a self-signed certificate `cert.pem` and a private key `private.key`. You will use these files to generate your `mongodb_tls.pem` file which contains both the certificate and private key to start `MongoDB` using TLS.

Issue the following commands to generate the self-signed certificate and private key. You will be prompted to enter information pertinent to the certificate. Ensure that you set the Common Name (CN) as `localhost` the prompt appears.
...
Next, run the following command to concatenate the self-signed certificate and private key to create a single `.pem` file to start `MongoDB` using TLS.
...
Use the Java `keytool` to add the certificate to the truststore. The password for the truststore is `mongodb`.

[role='command']
```
keytool -import -trustcacerts -keystore assets/security/truststore.p12 -storepass mongodb -storetype PKCS12 -alias mongo -file assets/security/cert.pem
```
...
Execute the following commands to build an image `FROM mongo` and run it in a Docker container using the provided `MongoDB` config (the assets folder will be copied into `/etc/mongodb` in the Docker container):
...
[role='command']
```
winpty docker exec -it mongo-guide bash
```
--
...
[role='command']
```
mongo --tls --tlsCAFile /etc/mongodb/security/cert.pem --host localhost
use testdb
...
Test the `CREATE` operation through the OpenAPI UI at the `POST /crew` endpoint with the following as the request body:
```
{
  "name": "Member1",
  "rank": "Officer",
...
Test the `READ` operation from the OpenAPI UI at the `GET /crew` enpoint.
...
Test the `UPDATE` operation from the OpenAPI UI at the `PUT /crew/\{id\}` endpoint by specifying the `\<<ID>>` you saved from the `CREATE` section in the id parameter and setting the request body as:
```
{
  "name": "Member1",
  "rank": "Captain",
...
Test the `DELETE` operation from the OpenAPI UI at the `DELETE /crew/\{id\}` endpoint by specifying the `\<<ID>>` you saved from `CREATE` section in the id parameter.
