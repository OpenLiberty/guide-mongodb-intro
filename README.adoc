...
This microservice connects to `MongoDB` using Transport Layer Security (TLS) and injects a `MongoDatabase` instance into the service through a CDI producer.  Additionally, MicroProfile Config is used to make configuring the `MongoDB` driver simple.
